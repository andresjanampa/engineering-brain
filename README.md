...
Every provider request passes through a local outbound gate before transport. The gate evaluates five fixed BLOCK policies: `SYS_REMOTE_COMPLETE_REPOSITORY`, `SYS_REMOTE_RAW_SNAPSHOT`, `SYS_REMOTE_SOURCE_BODIES`, `SYS_REMOTE_SECRETS`, and `SYS_REMOTE_ABSOLUTE_PATHS`. Providers accept only an `ApprovedReasoningRequest`, which represents an allowed `Exact` assessment of the complete model-visible request and carries a versioned, length-prefixed SHA-256 fingerprint. A pre-run CALL #2 assessment is `Projected` because the live CALL #2 bytes do not exist yet; it never grants transport authority. Historical preview schema 1, initiative schema 2, and live-result schema 2 artifacts load missing assessments as `NotRecorded`, never as allowed. Current artifact versions are preview schema 2, initiative schema 3, and live-result schema 3.

Known-secret checks use only command-scoped environment values whose variable names contain a supported secret token and whose trimmed values are at least eight characters and are not frozen common labels or placeholders. Values remain memory-only and are never written into findings, diagnostics, or persisted assessments. Provider failures are exposed and persisted only through fixed failure codes and messages.
...
See [the initial architecture decision](docs/decisions/0001-local-first-evidence-first.md), [the project-aware analysis decision](docs/decisions/0002-project-aware-semantic-analysis.md), [the incremental analysis decision](docs/decisions/0003-incremental-analysis.md), [the deterministic Project Memory decision](docs/decisions/0004-deterministic-project-memory.md), [the initiative-analysis decision](docs/decisions/0005-llm-initiative-analysis.md), [the evaluation and preview decision](docs/decisions/0006-evaluation-and-remote-preview.md), [the lexical ranking decision](docs/decisions/0007-retrieval-ranking.md), [the live model evaluation decision](docs/decisions/0008-live-model-evaluation.md), [the reviewed concept reranking decision](docs/decisions/0009-reviewed-concept-reranking.md), [the reviewed concept lifecycle decision](docs/decisions/0010-reviewed-concept-lifecycle.md), [the complete security policy decision](docs/decisions/0011-complete-security-policy-coverage.md), and [the architecture overview](docs/architecture/README.md) for the boundaries that guide future work.
